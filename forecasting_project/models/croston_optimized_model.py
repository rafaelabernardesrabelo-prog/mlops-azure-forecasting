...
class CrostonOptimizedModel(AbstractForecastingModel):
    ...
    def fit(self, target_series: pd.Series, covariates: Optional[pd.DataFrame] = None):
        # Instantiate CrostonOptimized with prepared parameters
        model = CrostonOptimized(**self.model_params)
    ...
    @classmethod
    def load_model(cls, path: str) -> 'CrostonOptimizedModel':
