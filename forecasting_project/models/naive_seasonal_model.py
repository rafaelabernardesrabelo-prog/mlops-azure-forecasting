...
class SeasonalNaiveModel(AbstractForecastingModel):
    ...
    def fit(self, target_series: pd.Series, covariates: Optional[pd.DataFrame] = None):
        ...
        # Instantiate SeasonalNaive with prepared parameters
        model = SeasonalNaive(**self.model_params)
    ...
    def save_model(self, path: str):
        # Note: StatsForecast models can be complex to pickle directly.
        # Saving the wrapper instance is a practical approach.
        with open(path, 'wb') as f:
            pickle.dump(self, f)

    @classmethod
    def load_model(cls, path: str) -> 'SeasonalNaiveModel':
