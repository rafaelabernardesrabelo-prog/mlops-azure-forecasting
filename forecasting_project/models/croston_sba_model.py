...
class CrostonSBAModel(AbstractForecastingModel):
    ...
    def fit(self, target_series: pd.Series, covariates: Optional[pd.DataFrame] = None):
        # Instantiate CrostonSBA with prepared parameters
        model = CrostonSBA(**self.model_params)
    ...
    @classmethod
    def load_model(cls, path: str) -> 'CrostonSBAModel':
