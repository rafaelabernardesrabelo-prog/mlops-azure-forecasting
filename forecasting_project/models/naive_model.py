...
class NaiveModel(AbstractForecastingModel):
    ...
    def fit(self, target_series: pd.Series, covariates: Optional[pd.DataFrame] = None):
        # Instantiate Naive with prepared parameters
        model = Naive(**self.model_params)
    ...
    @classmethod
    def load_model(cls, path: str) -> 'NaiveModel':
